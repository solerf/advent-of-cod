...
//only item type carried by all three Elves. That is, if a group's badge is item type B, then all three Elves will have item type B somewhere in their rucksack, and at most two of the Elves
...
var priorities = createPrioritiesMap()
...
func part01(lines *[][]string) {
  ...
  for _, sack := range *lines {
    splitAt := len(sack) / 2
    firstSack := toHash(sack[:splitAt])
    secondSack := toHash(sack[splitAt:])

    for item, _ := range firstSack {
      if _, exists := secondSack[item]; exists {
...
func part02(lines *[][]string) {
  ...
  for i := 0; i < len(sacks); i = i + 3 {
    firstSack := toHash(sacks[i])
    secondSack := toHash(sacks[i+1])
    thirdSack := toHash(sacks[i+2])

    for item, _ := range firstSack {
      _, existsSecond := secondSack[item]
      _, existsThird := thirdSack[item]
      if existsSecond && existsThird {
        prio := priorities[item]
        sacksSum += prio
      }
    }
...
func toHash(sack []string) map[string]none {
  sackItems := make(map[string]none)
  for _, item := range sack {
    if _, exits := sackItems[item]; !exits {
      sackItems[item] = isNone
    }
  }
  return sackItems
}

func createPrioritiesMap() map[string]int {
  p := make(map[string]int)
  for i := 1; i <= 26; i++ {
    lower := string(rune('a' - 1 + i))
    upper := string(rune('A' - 1 + i))
    p[lower] = i
    p[upper] = i + 26
  }
  return p
